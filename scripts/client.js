...
/******************* ROUTES *****************************/
const API_URL_PACKAGES = 'https://tendertouchapi.xyz/packages';
const API_URL_ADDONS = 'https://tendertouchapi.xyz/addons';
const API_URL_SETTINGS = 'https://tendertouchapi.xyz/settings';
const API_URL_APPTSLOTS = 'https://tendertouchapi.xyz/apptSlots';
const API_URL_JOBS = 'https://tendertouchapi.xyz/jobs';
const API_URL_PRICES = 'https://tendertouchapi.xyz/stripe-price';
const API_URL_PAYMENTINTENT = 'https://tendertouchapi.xyz/create-payment-intent';
...
async function getCarExteriorAddons() {
    let arr = [];

    await fetch(API_URL_ADDONS).then(res => res.json()).then(addons => {
        addons.forEach(a => {
            if(
			a.name === "Car - Interior Vacuum / Wipedown" ||
            a.name === "Car - Engine Detail" ||
            a.name === "Car - Clay Bar" ||
            a.name === "Car - HL Restoration") {
                arr.push(a);
            }
...
async function getTruckExteriorAddons() {
    let arr = [];

    await fetch(API_URL_ADDONS).then(res => res.json()).then(addons => {
        addons.forEach(a => {
            if(
			a.name === "Truck - Interior Vacuum / Wipedown" ||
            a.name === "Truck - Engine Detail" ||
            a.name === "Truck - Clay Bar" ||
            a.name === "Truck - HL Restoration") {
                arr.push(a);
            }
...
async function getCarInteriorAddons() {
    let arr = [];

    await fetch(API_URL_ADDONS).then(res => res.json()).then(addons => {
        addons.forEach(a => {
            if(
			a.name === "Car - Exterior Wash" ||
			a.name === "Car - Dog Hair Removal (Price is an estimate, subject to additional fees on site)") {
                arr.push(a);
            }
...
async function getTruckInteriorAddons() {
    let arr = [];

    await fetch(API_URL_ADDONS).then(res => res.json()).then(addons => {
        addons.forEach(a => {
            if(
			a.name === "Truck - Exterior Wash" ||
			a.name === "Truck - Dog Hair Removal (Price is an estimate, subject to additional fees on site)") {
                arr.push(a);
            }
...
async function getCarGeneralAddons() {
    let arr = [];

    await fetch(API_URL_ADDONS).then(res => res.json()).then(addons => {
        addons.forEach(a => {
            if(a.name === "Car - Spray Wax") {
                arr.push(a);
            }
        })
    });
...
async function getTruckGeneralAddons() {
    let arr = [];

    await fetch(API_URL_ADDONS).then(res => res.json()).then(addons => {
        addons.forEach(a => {
            if(a.name === "Truck - Spray Wax") {
                arr.push(a);
            }
...
async function getCarFullAddons() {
    let arr = [];
...
async function getTruckFullAddons() {
    let arr = [];
...
p1.addEventListener('click', async () => {
    ...
    typeCar.addEventListener('click', async () => {
        ...
        const specUpgrades = await getCarGeneralAddons();
        const addonHTML = await getAddonHTML(specUpgrades);
    ...
    typeTruck.addEventListener('click', async () => {
        ...
        const package = await getPackageInfo(name);
        const specUpgrades = await getTruckGeneralAddons();
        const addonHTML = await getAddonHTML(specUpgrades);
...
p2.addEventListener('click', async () => {
    ...
    typeCar.addEventListener('click', async () => {
        ...
        const package = await getPackageInfo(name);
        const specUpgrades = await getCarExteriorAddons();
        const addonHTML = await getAddonHTML(specUpgrades);
    ...
    typeTruck.addEventListener('click', async () => {
        ...
        const package = await getPackageInfo(name);
        const specUpgrades = await getTruckExteriorAddons();
        const addonHTML = await getAddonHTML(specUpgrades);
...
p3.addEventListener('click', async () => {
    ...
    typeCar.addEventListener('click', async () => {
        ...
        const package = await getPackageInfo(name);
        const specUpgrades = await getCarInteriorAddons();
        console.log(specUpgrades);
        const addonHTML = await getAddonHTML(specUpgrades);
    ...
    typeTruck.addEventListener('click', async () => {
        ...
        const package = await getPackageInfo(name);
        const specUpgrades = await getTruckInteriorAddons();
        const addonHTML = await getAddonHTML(specUpgrades);
...
p4.addEventListener('click', async () => {
    ...
    typeCar.addEventListener('click', async () => {
        ...
        document.querySelector('#addOn-form-fulldetail').scrollIntoView();

        const typeName = typeCar.id;
        const name = baseName + ' - ' +  typeName;
        const package = await getPackageInfo(name);

        const specUpgrades = await getCarFullAddons();
        const addonHTML = await getAddonHTML(specUpgrades);
    ...
    typeTruck.addEventListener('click', async () => {
        ...
        const specUpgrades = await getTruckFullAddons();
        const addonHTML = await getAddonHTML(specUpgrades);
...
async function displayOrderForm(date, hour, package, addonArray) {          
    ...
    document.querySelector('.submit-customer-details').addEventListener('click', async (event) => {
        ...
        if(valid) {
            ...
            const x = document.querySelector(".order-customer-details");
            const y = document.querySelector(".order-confirmation-details");
            y.innerHTML = '';
            x.innerHTML = `<p class='estimated-price'>Estimated Total - ${subtotal}</p>
                            <p class='estimated-price'>Price is subject to additional fees based on the condition of your vehicle at the time of service.
                            All prices listed are estimated based on average cases.</p>`;
            displayOrderConfirmation(customerData);
            updateAppointmentAvailability(customerData);
            postJob(customerData);
        }
...
async function updateAppointmentAvailability(customerData) {
    ...
    startHour = parseInt(startHour) - 3;
